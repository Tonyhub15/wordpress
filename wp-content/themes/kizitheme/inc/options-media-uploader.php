...
function optionsframework_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
	...
	$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" placeholder="' . __('No has seleccionado ningun archivo', 'optionsframework') .'" />' . "\n";
	if ( $value == '' ) {
		$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( 'Subir', 'options_framework_theme' ) . '" />' . "\n";
	} else {
		$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __( 'Eliminar', 'options_framework_theme' ) . '" />' . "\n";
	}
